                    ...
                    case R.Tilde:
                        {
                            ...
                            var idToken = _reader.ReadLoose(R.Text, "carrier rank");
                            int rank;
                            if (!int.TryParse(idToken.Value, out rank) || rank < 0)
                                SyntaxError(idToken, "Carrier rank must be a non-negative integer.");
                            _query.Carrier.Rank = rank;
                        }
                        break;
                    // match carrier
                    case R.Equal:
